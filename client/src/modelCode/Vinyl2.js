...
import { useGLTF, PerspectiveCamera, SpotLight, OrbitControls, PointMaterial, Point, Points } from "@react-three/drei";
...
export function Model2({ vinylPlay, setVinylPlay, setCurrentPlaying, setCurrentState, props }) {
  ...
  useFrame(
    (state, delta) => {
      ...
      if (clicked) {
        ...
        if (refParticle3.current.position.y >= 1.5 || refParticle.current.position.y <= 0) positionAux *= -1;
      }
    },
    [clicked]
  );
  ...
  return (
    <group {...props} dispose={null}>
      ...
      <Points>
        <PointMaterial vertexColors size={35} sizeAttenuation={false} depthWrite={true} toneMapped={false}>
          <Point ref={refParticle} color={"orange"} position={[-1, 0, -1]} />
          <Point ref={refParticle2} color={"orange"} position={[-1, 0.5, -1]} />
          <Point ref={refParticle3} color={"orange"} position={[-1, 1, -1]} />
        </PointMaterial>
      </Points>
